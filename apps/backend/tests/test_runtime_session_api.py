...
def test_runtime_router_reads_service_from_app_state() -> None:
    with TestClient(app_with(None)) as client:
        response = client.post(
            "/runtime/sessions",
            headers=headers(),
            json=start_body(),
    ...
    assert response.json()["detail"]["code"] == "runtime_session_service_unavailable"


def test_v1_runtime_request_is_rejected_as_an_explicit_conflict() -> None:
    service = FakeRuntimeSessionService()
    with TestClient(app_with(service)) as client:
        response = client.post(
            "/runtime/sessions",
...
def test_unknown_runtime_protocol_version_is_rejected_as_unprocessable() -> None:
    service = FakeRuntimeSessionService()
    with TestClient(app_with(service)) as client:
        response = client.post(
            "/runtime/sessions",
            headers=headers(version="999"),
            json=start_body(),
        )
