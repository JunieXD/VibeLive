...
- 状态：`Accepted`
- 日期：2026-07-23
- 决定：FastAPI 承担本地 ASR、近期上下文、AI 编排、Provider 接入和弹幕处理；Python 项目使用 `uv` 管理依赖和锁文件。
...
- 状态：`Accepted`
- 日期：2026-07-23
- 决定：生成弹幕的观察上下文由近期画面帧、用户文字、本地 ASR 文本及必要的公开房间事件组成。
...
- 影响：每条 AI 弹幕必须归属于已有观众 ID；批量请求不能把人格和记忆合并成无身份的统一声音。模型调用次数和调度拓扑不属于产品不变量。

### D-015：发言与调度算法暂不固定

- 状态：`Accepted`
- 决定：当前不固定何时触发弹幕、选择哪些观众、是否允许观众互相接话、一次请求包含几个观众或记忆何时更新。
- 影响：实现只需保证房间事件有序、观众状态隔离、队列有界和过期结果丢弃。任何调度方案都必须服从 D-014 的身份连续性要求。
...
- 状态：`Accepted`
- 决定：普通配置、观众档案和观众记忆保存在本地。模型凭据由 Electron `safeStorage` 保存，经短期鉴权的本地通道按会话注入 FastAPI 内存。两端使用带会话标识的结构化本地日志。
...
- 状态：`Open`
- 需要回答：画面使用 JPEG、WebP 或其他格式，音频使用何种 PCM 参数，是否需要共享内存或仅使用 WebSocket 二进制消息。

### Q-002：观众发言和调用如何调度？

- 状态：`Open`
- 需要回答：何时产生新反应、挑选哪些观众、批量还是独立调用、是否允许观众彼此接话，以及如何避免内部对话挤占用户交流。
- 约束：无论采用哪种算法，每个结果都必须绑定已有观众 ID，并使用该观众自己的状态和记忆。
...
| 使用 StepAudio 云端 ASR | 被 D-005 取代 |
| 固定 32 个独立人格调用 | D-014 只要求独立逻辑身份，调用方式由 Q-002 决定 |
| 固定的人群导演模型 | 是否需要调度模型由 Q-002 决定 |
